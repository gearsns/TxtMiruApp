export const narou2html = (text: string) => {
    ...
    for (const line of text.split(/\n/)) {
        ...
        const line_item: { type: string, text: string, start?: number }[] = []
        for (const target of line.split(/(｜|\||[《（\()].*?[》）\)]|<.+\|.+>)/)) {
            ...
            const splitRuby2 = (text: string, split_type: number) => {
                let item_type = "ruby"
                let r
            ...
            const autoDetectRubyBase = (text: string) => {
                let item_type = "ruby"
                let r
                if ((text.match(/[　 ]/g) || []).length >= 2) {
                    item_type = "text"
        ...
        for (const item of line_item) {
            if (item.type === "ruby" && line_item[item.start || 0]) {
                line_item[item.start || 0].type = "ruby_rb"
            }
        }
        ret.push(line_item)
    }
    ...
    for (const line_item of ret) {
        ...
        for (const item of line_item) {
            ...
            } else if (item.type === "ruby" && line_item[item.start || 0]) {
                html_arr.push(`<ruby>${line_item[item.start || 0].text}<rt>${item.text}</rt></ruby>`)
